...
//					addCustomData(customData5Metrics, 12, dataList);
	...
	public Map<String, List<Float>> getReportData(long testId, String key, boolean onlyTotal, int interval) {
		...
		reportDataFiles.forEach(each -> resultMap.put(buildReportName(key, each), getFileDataAsList(each, interval)));
		return resultMap;
	}
	...
	private String buildReportName(String key, File file) {
		...
		if (key.startsWith("User_defined")) {
			return reportName;
		}
		...
		if (SingleConsole.isPerfTestInterestingStatistics(baseName[0]) && baseName.length >= 2) {
			String recordDesc = baseName[1];
			reportName = recordDesc.substring(recordDesc.indexOf("_") + 1);
		}
		return reportName;
	}
